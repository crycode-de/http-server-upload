...
let pathMatchRegExp = process.env.PATH_REGEXP
    ? new RegExp(process.env.PATH_REGEXP)
    : /^[a-zA-Z0-9-_/]*$/;
let maxFileSize =
    (parseInt(process.env.MAX_FILE_SIZE, 10) || 200) * 1024 * 1024;
    ...
    if (arg.startsWith("--")) {
        ...
        if (typeof val === "undefined" && key !== "--help") {
            ...
            if (typeof val === "undefined") {
                console.warn(
                    `WANRING: No value given for command line argument: ${key}`
                );
                continue;
            }
        }
        ...
        if (req.url == "/upload" && req.method.toLowerCase() === "post") {
            ...
            form.parse(req, (err, fields, files) => {
                ...
                if (token && fields.token !== token) {
                    ...
                    files.uploads.forEach(
                        (file) => file && fs.unlink(file.filepath)
                    );
                    return res.end();
                }
